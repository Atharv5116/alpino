...
# Employee compensation fields to keep behind permission level 1.
...
def setup_employee_salary_field_permissions():
	"""Move Employee salary fields to permlevel 1 and grant that level to the HR roles only."""
	from frappe.custom.doctype.property_setter.property_setter import make_property_setter
	from frappe.permissions import add_permission, update_permission_property

	try:
		meta = frappe.get_meta("Employee")
		for fieldname in EMPLOYEE_SALARY_FIELDS:
		...
		frappe.db.commit()
		print("✅ Employee salary fields restricted to permlevel 1 (HR roles only)")
