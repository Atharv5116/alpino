...
def upload_onboarding_file():
	"""
	Guest-accessible file upload endpoint for the Employee Onboarding webform.
	The default Frappe upload_file API requires authentication; this allows
	unauthenticated candidates to upload their documents.
	"""
	onboarding_name = (
		frappe.form_dict.get("onboarding")
		or frappe.form_dict.get("employee_onboarding_name")
	)

	if not onboarding_name:
		frappe.throw(_("Missing onboarding reference. Please use the link from your email."))

	if not frappe.db.exists("Employee Onboarding", onboarding_name):
		frappe.throw(_("Invalid onboarding reference."))

	# Validate that the file was actually sent
	...
	if len(content) > 10 * 1024 * 1024:
		frappe.throw(_("File size must not exceed 10 MB."))

	# Save the file attached to the Employee Onboarding record
