 ...
 *
 * Shows the items held out of a Purchase Inward, the one reminder that covers them, and
 * lets the Store, QC or Admin team select held items and release them. A release raises a
 * Purchase QC for exactly those items (alpinos.purchase.quarantine.release_items), after
 * which they follow the normal QC -> GRN -> Purchase Invoice path.
 */
...
var PurchaseQuarantineView = class {
	...
	render() {
		...
		ro('.field-entire-inward', 'entire_inward', 'Entire Inward', 'Data', null, cint(doc.entire_inward) ? __('Yes') : __('No'));
	...
	apply_state() {
		...
		let note = '';
		if (this.docname) {
			if (status === 'Released') note = __('Every item has been released. Each went to its own Purchase QC, then GRN and Purchase Invoice.');
			...
			() =>
				frappe.call({
					...
					callback(r) {
						...
						frappe.show_alert({
							message: __('Released — Purchase QC {0} raised', [r.message.purchase_qc]),
							indicator: 'green',
						}, 6);
						me.load(me.docname);
					},
				...
				options: qcs.map((q) => `
					<div style="display:flex;align-items:center;justify-content:space-between;gap:12px;padding:8px 0;border-bottom:1px solid var(--border-color);">
						<div>
							...
							<div class="text-muted" style="font-size:12px;">${esc(q.source === 'inward' ? __('Items that were not quarantined') : __('Released from quarantine'))}${q.qc_status ? ' · ' + esc(__(q.qc_status)) : ''}</div>
						</div>
