...
# Copyright (c) Alpinos custom report — safe None-handling vs core Shift Attendance
...
def update_data(data, filters):
	consider = filters.get("consider_grace_period")
	for d in data:
		update_late_entry(d, consider)
		update_early_exit(d, consider)
...
def update_late_entry(entry, consider_grace_period):
	"""Same as HRMS Shift Attendance, but never subtract None from datetime (avoids TypeError)."""
	entry.late_entry_hrs = None
	if consider_grace_period:
		if entry.late_entry and entry.in_time is not None and entry.shift_start is not None:
	...
	elif entry.in_time is not None and entry.shift_start is not None and entry.in_time > entry.shift_start:
		entry.late_entry = 1
...
def update_early_exit(entry, consider_grace_period):
	"""Same as HRMS Shift Attendance, but never subtract involving None (avoids TypeError)."""
	entry.early_exit_hrs = None
	if consider_grace_period:
		if entry.early_exit and entry.out_time is not None and entry.shift_end is not None:
	...
	elif entry.out_time is not None and entry.shift_end is not None and entry.out_time < entry.shift_end:
		entry.early_exit = 1
