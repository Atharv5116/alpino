...
def _normalize_obm_quotation_args(args, doc=None):
	"""Resolve Buyer Master quotations to Customer before ERPNext tax/pricing lookup."""
	...
	if doctype != "Quotation" or quotation_to != "Buyer Master":
		return args, doc
	...
	if not customer and obm_name:
		customer = frappe.db.get_value("Buyer Master", obm_name, "customer")
...
def get_party_details(
	...
	"""Intercept party_type='Buyer Master' and resolve to linked Customer."""
	if party_type == "Buyer Master" and party:
		customer = frappe.db.get_value("Buyer Master", party, "customer")
		if customer:
			party_type = "Customer"
			party = customer
		else:
